...
def lcs_sub(a,b):
    ...
    return s
data=json.load(open(f"{HERE}/batches_full/batch_src_004.json",encoding='utf-8'))
for e in data:
    if e['serial'] in ('#0558','#1330'):
        r=G.RECOMMENDS[e['serial']]
        print('====',e['serial'])
        for fn,ft in _field_texts(e):
            sub=lcs_sub(r,ft)
            if len(sub)>=8: print('  ',fn,len(sub),'「'+sub+'」')
